...
class PoolMemberType:
    """
    Impementation of lookup table between Sample Pool member and identified
    read_group_tags for a given READ_LABEL.

    :ivar identifiers: Set of reference IDs to parent experiment record.
        This block is intended to replace the use of the less
        structured RefNameGroup identifiers.
    :ivar read_label:
    :ivar refname: Identifies a record by name that is known within the
        namespace defined by attribute "refcenter" Use this field when
        referencing an object for which an accession has not yet been
        issued.
    :ivar refcenter: The center namespace of the attribute "refname".
        When absent, the namespace is assumed to be the current
        submission.
...
class SampleDescriptorType:
    ...
    :ivar pool: Identifies a list of group/pool/multiplex sample
        ...
        or multiplex, but is continues to receive its own accession and
...
class ExperimentType:
    ...
    :ivar identifiers: List of primary and alternate identifiers
        including those records replacing or replaced by this record.
    ...
    :ivar study_ref: The STUDY_REF descriptor establishes the
        relationship of the experiment to the parent study.  This can
        either be the accession of an existing archived study record, or
        a reference to a new study record in the same submission (which
        does not yet have an accession).
    ...
    class StudyRef:
        """
        :ivar identifiers: Set of reference IDs to parent study record.
            This block is intended to replace the use of the less
            structured RefNameGroup identifiers.
